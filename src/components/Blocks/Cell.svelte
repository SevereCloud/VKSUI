<style>
  ...
  .Cell--ios :global(.Cell__in) {
    ...
    transition: background-color 0.15s ease-out,
      transform 0.6s var(--ios-easing);
  }

  .Cell--ios .Cell__before-in > :global(*) :global(.Icon) {
    padding: 10px 16px 10px 0;
  }

  .Cell--ios.Cell--m .Cell__before-in > :global(*) :global(.Avatar) {
  ...
  .Cell--ios.Cell--l .Cell__before-in > :global(*) :global(.Avatar) {
  ...
  .Cell--android .Cell__before-in > :global(*) :global(.Icon),
  ...
  .Cell--android .Cell__before-in > :global(*) :global(.Avatar) {
    padding: 8px 12px 8px 0;
  }

  .Cell--android.Cell--l .Cell__before-in > :global(*) :global(.Avatar) {
    padding: 12px 12px 12px 0;
  }
  ...
  const onDragMove = ({ detail }) => {
    ...
    siblings.forEach((sibling, siblingIndex) => {
      ...
      if (dragStartIndex < siblingIndex) {
        if (
          rootGesture.bottom >
          siblingGesture.top + siblingGesture.height / 2
        ) {
        ...
        if (
          rootGesture.top <
          siblingGesture.bottom - siblingGesture.height / 2
        ) {
          if (dragDirection === 'up') {
            sibling.style.transform = 'translateY(100%)';
          }
